...
public class PrivacyAlert : UIElement{
    ...
    private void selectCenterBt(bool selected){
        if (selected){
            sureButton.GetComponent<Image>().sprite = Resources.Load("Images/button_green", typeof(Sprite)) as Sprite;
        } else{
            sureButton.GetComponent<Image>().sprite = Resources.Load("Images/button_gray", typeof(Sprite)) as Sprite;
        }
    }
